...
class OfflineSyncService {
  ...
  Future<void> queueFavoriteToggle(String quoteId, bool isFavorite) async {
  ...
  Future<void> _executeOperation(SyncOperation operation) async {
    switch (operation.type) {
      case SyncOperationType.createQuote:
        await _firestoreDataSource.addQuoteFromJson(operation.data);
        break;
      case SyncOperationType.updateQuote:
        await _firestoreDataSource.updateQuoteFromJson(
          operation.quoteId!,
          operation.data,
        );
        break;
      case SyncOperationType.deleteQuote:
        await _firestoreDataSource.deleteQuote(operation.quoteId!, '');
        break;
      case SyncOperationType.toggleFavorite:
        ...
        if (userId != null && operation.quoteId != null) {
          if (isFavorite) {
            await _firestoreDataSource.addFavorite(userId, operation.quoteId!);
          } else {
            await _firestoreDataSource.removeFavorite(userId, operation.quoteId!);
          }
        }
        break;
      case SyncOperationType.updateProfile:
        await _firestoreDataSource.saveUserFromJson(operation.data);
        break;
    }
  }
